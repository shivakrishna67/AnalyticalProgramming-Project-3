def exploratoryDataAnalysis():
    """Performs the exploratory data analysis """
    print("*******************************************************")
    print("EDA Starts here")
    print("""In order to go on with this portion of the study, we will first run a summary statistic for each of the characteristics, look for any outliers or NULL values that could have an impact on our findings, and then provide graphical analyses for each attribute using both Matplotlib and Seaborn.\n
    ...
    print("lets drop the attributes that we are not considering to answer the above mentioned research")
    print("=============================================================================================")
    data=data.drop(['zip_code','latitude','longitude','location','cross_street_name','off_street_name','contributing_factor_vehicle_2','contributing_factor_vehicle_3','contributing_factor_vehicle_4','contributing_factor_vehicle_5','vehicle_type_code2','vehicle_type_code_3','vehicle_type_code_4','vehicle_type_code_5'],axis=1)
    print("Data after dropping the unwanted attributes")
    print("=============================================================================================")
    print(data)
    print("*********************************************************************************************")
    print("""Since we've successfully imported the data, we'll proceed to look through the data if there are null values. Null values may impact our research so we'll have to be keen""")
    #Looking for the number of null values
    print("Looking for the number of null values")
    print(data.isnull().sum())
    print("From the above analysis we can see that there were null values only in 4 columns namely on_street_name, contributing_factor_vehicle_1 , vehicle_type_code1 and borough.")
    #sumamry statistics of the data
    print("""========================================================
             sumamry statistics of the data
             =========================================================""")
    print(data.describe())
    print("Observations : From the above statistics we can observe the median mode and mean values for all numerical columns and understand the distribution of the data")
    print("""===========================================================
    ...
    print("""From the above data it is found that there are 814 unique values in the crash date attribute.""")
    print("""*********************************************************
            Crash Time 
            *********************************************************""")
    print(data['crash_time'].describe())
    print("from the above data it is found that there are 1440 unique values in the crash time attribute.")
    print("""*********************************************************
            Borough 
            *********************************************************""")
    ...
    plt.tight_layout()
    plt.show()
    print("From the above data it is found that there are 5 unique values in the Borough attribute.Which says that there are 5 different borough are present.")
    # On Street Name
    print("""*********************************************************
            On Street name 
            *********************************************************""")
    print(data['on_street_name'].describe())
    print("From the above data it is found that there are 7727 unique streets involved in crash in New York City.")
    print("""*********************************************************
    ...
    plt.tight_layout()
    plt.show()
    print("""We have total 180030 observations and from the above statistical data we can observe that the average no of persons injured in the crashes is 1 person per two crashes.From the above distribution, we can say that mostly number of persons injured were falling in the bin of 0 - 5. 
     """)
    ...
    print("here we can find the average and std deviation min and max no of people can be killed per a crash as below")
    ...
    plt.tight_layout()
    plt.show()
    print("From the above distribution we can say that the maximum number of people killed per crash is 3 people.")

    #now lets convert the Crash date into a date data type
    data['crash_date'] = pd.to_datetime(data['crash_date'])
    print("Below are the data types for all attributes")
    print(data.dtypes)
    print("""In the below step we extract the day month and year from the crash date attribute and also changed to its respective month day names to make easier to analyse the data well.""")

    import calendar
    #Exctracting the year, month, day to analyse the seasonal and temporal time analysis of crashes
    print("Exctracting the year, month, day to analyse the seasonal and temporal time analysis of crashes")
    ...
    print("""************************************************
             ...
             ************************************************""")
    print("""First I will get summary statistics on the attribute Year. Then, since Year is an ordinal variable, I will generate bar plots using both Matplotlib and Seaborn.""")
    ...
    year_bar_plot
    plt.show()
    print("From the above graphs it was evident that a large no of crashes happened during 2021.")
    print("""**************************************************
    ...
    plt.show()
    print("As per the above plots it was evident that June marks the highest and then followed by May . To be specific Quarter 4 has the least no of crashes.The reason behind this may be that as it was a winter season people are less tend to travel leading to less no of crashes.")
    ...
    #show the bar/count plot
    plt.show()
    print("From the above plots It is evident that Tuesday marks the highest crashes day followed by wednesday and Thursday with slight difference.")
    ...
    borough
    plt.show()
    print("Based on the data shown above, it is obvious that Brooklyn stood at first place for happening most crashes followed by Queens and STATEN ISLAND beinhg the least and indicating the safe place in terms of travelling.")
    ...
    plt.show()
    print("As there were very large number of street Here I displayed only top 5 streets of having large number of crashes.Belt parkway experience the most no of vehicle crashes follwed by long island express highway.")
    ...
    plt.ylabel("Total Injuries")
    plt.show()
    print("From the above plots Tuesday and Wednesday has the highest no of injuries in a week but all are having slight difference.")
    ...
    **************************************************************************""")
    ...
    plt.ylabel("Total Deaths")
    plt.show()
    print("As earlier July has most no of deaths and the months of last quarter i.e.,October, November and December and september also stood at least no of deaths.")
    ...
    plt.ylabel("Total Deaths")
    plt.show()
    print("From the above plots, it is evident that weekdays Thursday has highest number of deaths followed by the Tuesday.")
    ...
    cause
    plt.show()
    print("As there are large number of causes and unable to read from a messy visual I have shown top 5 causes in the plot.Driver Inattention/ Distraction is the main reason for the crashes if we set aside the unspecified reason.")
    ...
    **********************************************************""")
    ...
    plt.show()
    hour
    print("From the above plot it is very interesting to observe that all the crashes has happened from 2pm to 6pm .")
    ...
    *******************************************************""")
    ...
    plt.show()
    print("Brooklyn has more number of deaths followed by the Bronx.Staten Island is the borough with the least number of deaths.")
    ...
    *******************************************************************************""")
    ...
    plt.xticks(rotation=45, ha='right')
    plt.show()
    print("More number of people were killed because of the Alcohol involvement followed by the Aggressive driving.")
    print("""**********************************************************************
    Injuries v/s Deaths
    *************************************************************************""")
